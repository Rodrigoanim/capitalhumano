...
def save_transcription(result, filename):
    ...
    with open(vtt_path, "w", encoding="utf-8") as f:
        ...
        for i, word in enumerate(words):
            ...
            # Criar nova linha após aproximadamente 7 palavras ou no final
            if len(current_line) >= 7 or i == len(words) - 1:
                end_time = word["end"]
                
                # Converter milissegundos para formato VTT (HH:MM:SS.mmm)
...
def process_audio_transcription(video_id, video_title):
    ...
    if mark_as_transcribed(video_id):
        ...
        # Mostrar resultado
        st.success(f"Transcrições salvas em:\nTXT: {txt_path}\nVTT: {vtt_path}")
    ...
    if mode == "Manual":
        ...
        if selected_mp3:
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT you_id, titulo, autor, sumario FROM youtube_tab WHERE titulo = ? AND user_id = ?",
                    (video_title, user_id)
                )
                ...
                if video_info:
                    video_id, _, video_author, video_summary = video_info
                    ...
                    st.warning(f"Arquivo MP3 '{video_title}' encontrado, mas não há registro correspondente no banco de dados.")
            
            except Exception as e:
                st.error(f"Erro ao buscar informações do vídeo: {str(e)}")
