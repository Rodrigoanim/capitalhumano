...
class YouTubeMetadados:
    ...
    def coletar_metadados(self, url):
        """Coleta título, autor e descrição do vídeo usando requests e BeautifulSoup"""
        try:
    ...
    def adicionar_video(self, url, user_id):
        ...
        try:
            ...
            # Insere com o campo sumario
            self.cursor.execute('''
                INSERT INTO youtube_tab (
                    titulo, url, autor, user_id, 
                    sumario, insights, contraintuitivo, word_key, tools
                ) VALUES (?, ?, ?, ?, ?, '', '', '', '')
            ''', (
                titulo_filtrado,
                url,
...
def show_url_metadados():
    ...
    query = """
        SELECT you_id, user_id, titulo, autor, url, sumario
        FROM youtube_tab 
        WHERE user_id = ?
    """
    params = [user_id]
    ...
    if not df.empty:
        # Ajustar a exibição das colunas
        df_display = df[['titulo', 'autor', 'url', 'sumario']]
