...
def get_chat_response(prompt, transcription, video_url, mode="qa", temperature=0.7):
    ...
    try:
        ...
        # Padrões diferentes de timestamp que podem aparecer no texto
        timestamp_patterns = [
            r'\[(\d{2}:\d{2}:\d{2})\]',  # [00:00:00]
            r'(\d{2}:\d{2}:\d{2})',      # 00:00:00
        ]

        # Processar cada padrão de timestamp
        for pattern in timestamp_patterns:
            matches = re.finditer(pattern, content)
            
            # Lista para armazenar substituições a serem feitas
            replacements = []
            
            for match in matches:
                timestamp = match.group(1)  # Extrai o timestamp sem os colchetes
                youtube_time = convert_to_youtube_time(timestamp)
                youtube_link = f"{video_url}&t={youtube_time}"
                
                # Se o timestamp já não estiver entre colchetes, adiciona
            ...
            # Aplicar substituições do maior para o menor texto para evitar conflitos
